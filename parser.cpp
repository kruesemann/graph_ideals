...
void script_parse(DatabaseInterface * dbi, std::string * input) {
	...
	if (idealname.find("Bettis") == std::string::npos)
		dbi->generate_m2_scripts(&idealname, labeling == -1 ? 0 : GENERATORS[labeling], batch_size == -1 ? 2500 : batch_size, query_condition.empty() ? 0 : query_condition.c_str(), filename.empty() ? 0 : filename.c_str(), labeling == -1 ? 0 : LABELINGS[labeling], result);
	else
		FAIL("Generating M2 scripts", "Please choose an ideal name other than '" << idealname << "'. Any name containing 'Bettis' would interfere with the program.");
}
...
void results_parse(DatabaseInterface * dbi, std::string * input) {
	...
	int result = dbi->find_script_data(scriptID, &ideal, &query_condition, &datetime);

	if (result == -1)
		return;

	PROGRESS(1, "adding result data");
	if ((dbi->*INSERTERS[result])(&ideal, &query_condition, &datetime, result))
