...
#define scripts_text	"Enter 'scripts (-[labeling]) (-[batch size]) [ideal name] ([template name]) (-where \"[condition]\")' to generate Macaulay2 scripts based on a template.\n" \
						"\n" \
						"--Example: scripts -closed -2500 bei template.g6 -where \"type LIKE '%closed%'\"\n" \
						"\n" \
						...
						"\n" \
						"--The 'template name' must specify a relative path to a template for a Macaulay2 script (default is \"template.m2\").\n" \
						"\n" \
...
#define betti_text	"Enter 'betti (-[script id])' to add (Macaulay2-generated) Betti data to graphs for which Macaulay2 scripts were generated before.\n" \
					"\n" \
					"--Example: betti -1\n" \
					"\n" \
